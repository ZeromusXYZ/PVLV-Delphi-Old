...
uses packetdefs, Vcl.Grids, System.Classes, System.Contnrs, Vcl.ComCtrls,
...
Function AddPacketInfoToStringGrid(PD: TPacketData; SG: TStringGrid;
  var BlockName: String): Boolean;
...
CONST
  ...
  DataColors: Array [0 .. 7] of TColor = (clBlack, clRed, clGreen, clBlue,
    clPurple, clDkGray, clMaroon, clNavy);
...
Function GetREPosForRawByte(Pos: Integer): Integer;
CONST
  BeginOfData = Length(RawDataHeader1) + Length(RawDataHeader2) +
    (Length(CR) * 2) + RawDataRowHeaderSize;
VAR
  I, N: Integer;
Begin
  N := BeginOfData; // Base
...
Begin
  Case Cat Of
    ...
    1:
      Result := 'Melee Attack';
    2:
      Result := 'Ranged Attack Finish';
    3:
      Result := 'Weapon Skill Finish';
    4:
      Result := 'Magic Finish';
    5:
      Result := 'Item Finish';
    6:
      Result := 'Job Ability Finish';
    7:
      Result := 'Weapon Skill Start';
    8:
      Result := 'Magic Start';
    9:
      Result := 'Item Start';
    10:
      Result := 'Job Ability Start';
    11:
      Result := 'NPC Ability Finish';
    12:
      Result := 'Ranged Attack Start';
    13:
      Result := 'Pet Mob Ability Finish';
    14:
      Result := 'Job Ability DNC';
    15:
      Result := 'Job Ability RUN';

    // DSP stuff (not actually used as max value would be 15 with just 4 bits)
    29:
      Result := 'ACTION_ITEM_INTERRUPT';
    31:
      Result := 'ACTION_MAGIC_INTERRUPT';
    32:
      Result := 'ACTION_RANGED_INTERRUPT';
    33:
      Result := 'ACTION_MOBABILITY_START';
    35:
      Result := 'ACTION_MOBABILITY_INTERRUPT';
    37:
      Result := 'ACTION_RAISE_MENU_SELECTION';
  Else
...
Begin
  ...
  Case Reaction Of
    ...
    $01:
      Result := 'REACTION_MISS';

    $02:
      Result := 'REACTION_PARRY';
    ...
    $09:
      Result := 'REACTION_EVADE';

    $14:
      Result := 'REACTION_GUARD'; // mnk guard (20 dec)
...
Function AddPacketInfoIn0x028(PD: TPacketData; SG: TStringGrid): Integer;
// Returns last used Byte
VAR
  pSize: Byte;
  pActor: UInt32;
  pTargetCount: UInt16;
...
Begin
  ...
  AddSGRow(SG, $5, 'Actor', '0x' + IntToHex(pActor, 8) + ' - ' +
    IntToStr(pActor), 2);
...
VAR
  ...
  LType, LOffsetStr, LSubOffsetStr, LRangeOffsetStr, LName,
    LDescription: String;
...
Begin
  ...
  Try
    ...
    Begin
      ...
      If (BlockName <> CurrentBlock) and (CurrentBlock <> '') Then
        Continue;
      ...
      Begin
        ...
        If (Line.Count > 3) Then
          ...
          LDescription := '???'; // we're not actually using this (yet)
        ...
        If (LName = '') Then
        Begin
          LName := LType + ' @ 0x' + IntToHex(LOffset, 2) + ' ' +
            IntToStr(LOffset);
        End;
        ...
        Begin
          ...
          Begin
            ...
            Begin
              // OK switchval seems valid, next compare it
              If PD.GetBitsAtPos(LOffset, LSubOffset, LSizeOffset)
                = SwitchVal Then
              Begin
                BlockName := LDescription;
                AllowAutoSwitchBlock := False;
        ...
        Begin
          ...
          AddSGRow(SG, LOffset, LName, '0x' + IntToHex(PD.GetByteAtPos(LOffset),
            2) + '  ' + IntToStr(PD.GetByteAtPos(LOffset)));
        End
        Else If ((LType = 'byteflag') or (LType = 'flag')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 1;
          ColIndex := ColIndex + 1;
          If Assigned(UpdateActiveRE) Then
            MarkREBytes(UpdateActiveRE, LOffset, 1, DataCol(ColIndex));
          AddSGRow(SG, LOffset, LName, '0x' + IntToHex(PD.GetByteAtPos(LOffset),
            2) + '  ' + BytetoBit(PD.GetByteAtPos(LOffset)) + '  ' +
            IntToStr(PD.GetByteAtPos(LOffset)));
        End
        Else If ((LType = 'bit') or (LType = 'bool')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 1;
          ColIndex := ColIndex + 1;
          ...
          AddSGRow(SG, LOffset, LName,
            BoolToStr(PD.GetBitAtPos(LOffset, LSubOffset), True));
        End
        Else If ((LType = 'bits') or (LType = 'bitrange')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 1;
          If (LSizeOffset <= 0) Then
            LSizeOffset := 1;
          ColIndex := ColIndex + 1;
          If Assigned(UpdateActiveRE) Then
            MarkREBytes(UpdateActiveRE, LOffset, (LSizeOffset div 8) + 1,
              DataCol(ColIndex));
          AddSGRow(SG, LOffset, LName, '0x' + IntToHex(PD.GetBitsAtPos(LOffset,
            LSubOffset, LSizeOffset), 2) + ' - ' +
            IntToStr(PD.GetBitsAtPos(LOffset, LSubOffset, LSizeOffset)),
            (LSizeOffset div 8) + 1);
        End
        Else If ((LType = 'bitslookup') or (LType = 'bitrangelookup')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 1;
          If (LSizeOffset <= 0) Then
          ...
          AddSGRow(SG, LOffset, LName, '0x' + IntToHex(PD.GetBitsAtPos(LOffset,
            LSubOffset, LSizeOffset), 2) + ' - ' +
            IntToStr(PD.GetBitsAtPos(LOffset, LSubOffset, LSizeOffset)) + ' - '
            + NLU(LDescription).GetVal(PD.GetBitsAtPos(LOffset, LSubOffset,
            LSizeOffset)), (LSizeOffset div 8) + 1);
        End
        Else If ((LType = 'word') or (LType = 'uint16') or (LType = 'ushort') or
          (LType = 'w') or (LType = 'unsigned short')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 2;
          ColIndex := ColIndex + 1;
          ...
          AddSGRow(SG, LOffset, LName, '0x' + IntToHex(PD.GetWordAtPos(LOffset),
            4) + '  ' + IntToStr(PD.GetWordAtPos(LOffset)), 2);
        End
        Else If ((LType = 'int16') or (LType = 'short')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 2;
          ColIndex := ColIndex + 1;
          If Assigned(UpdateActiveRE) Then
            MarkREBytes(UpdateActiveRE, LOffset, 2, DataCol(ColIndex));
          AddSGRow(SG, LOffset, LName,
            '0x' + IntToHex(PD.GetInt16AtPos(LOffset), 4) + '  ' +
            IntToStr(PD.GetInt16AtPos(LOffset)), 2);
        End
        Else If ((LType = 'int') or (LType = 'int32') or (LType = 'i')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 4;
          ColIndex := ColIndex + 1;
          ...
          AddSGRow(SG, LOffset, LName,
            '0x' + IntToHex(PD.GetInt32AtPos(LOffset), 8) + '  ' +
            IntToStr(PD.GetInt32AtPos(LOffset)), 4);
        End
        Else If ((LType = 'dword') or (LType = 'long') or (LType = 'longword')
          or (LType = 'uint32') or (LType = 'u') or
          (LType = 'unsigned int')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 4;
          ColIndex := ColIndex + 1;
          ...
          AddSGRow(SG, LOffset, LName,
            ...
            IntToStr(PD.GetUInt32AtPos(LOffset)), 4);
        End
        Else If ((LType = 'float') or (LType = 'f')) Then
        Begin
          If (LOffset >= LastPos) Then
            LastPos := LOffset + 4;
          ColIndex := ColIndex + 1;
    ...
    Begin
      ColIndex := ColIndex + 1;
      If Assigned(UpdateActiveRE) Then
        MarkREBytes(UpdateActiveRE, I, 1, DataCol(ColIndex));
      AddSGRow(SG, I, '?Byte 0x' + IntToHex(I, 2) + ' ' + IntToStr(I),
        '0x' + IntToHex(PD.GetByteAtPos(I), 2) + '  ' +
        BytetoBit(PD.GetByteAtPos(I)) + '  ' + IntToStr(PD.GetByteAtPos(I)));
      I := I + 1;
    End;
