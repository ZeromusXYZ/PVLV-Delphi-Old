...
VAR
  I: Integer;
begin
  FilterSaveDialog.InitialDir := ExtractFilePath(Application.ExeName) +
    'filters\';

  CBOut.Clear;
  For I := 0 To NLU(LU_PacketOut).Count - 1 Do
    CBOut.Items.Add('0x' + IntToHex(NLU(LU_PacketOut).GetItem(I).ID, 3) + ' ' +
      NLU(LU_PacketOut).GetItem(I).Val);

  CBIn.Clear;
  For I := 0 To NLU(LU_PacketIn).Count - 1 Do
    CBIn.Items.Add('0x' + IntToHex(NLU(LU_PacketIn).GetItem(I).ID, 3) + ' ' +
      NLU(LU_PacketIn).GetItem(I).Val);
  ClearFilters;
end;
...
Begin
  ...
  For I := 0 To Length(PL.FilterOutList) - 1 Do
    LBOut.Items.Add('0x' + IntToHex(PL.FilterOutList[I], 3) + ' - ' +
      NLU(LU_PacketOut).GetVal(PL.FilterOutList[I]));

  For I := 0 To Length(PL.FilterInList) - 1 Do
    LBIn.Items.Add('0x' + IntToHex(PL.FilterInList[I], 3) + ' - ' +
      NLU(LU_PacketIn).GetVal(PL.FilterInList[I]));
...
Begin
  ...
  Try
    ...
    Begin
      ...
      Begin
        ...
        If (Val = 'hide') Then
          RBOutHide.Checked := True;
        If (Val = 'show') Then
          RBOutShow.Checked := True;
        If (Val = 'none') Then
          RBOutNone.Checked := True;
      End;
      If (Key = 'intype') Then
      Begin
        If (Val = 'off') Then
        ...
        If (Val = 'hide') Then
          RBInHide.Checked := True;
        If (Val = 'show') Then
          RBInShow.Checked := True;
        If (Val = 'none') Then
          RBInNone.Checked := True;
      End;
      If (Key = 'out') Then
      Begin
        If TryStrToInt(Val, V) Then
          LBOut.Items.Add('0x' + IntToHex(V, 3) + ' - ' + NLU(LU_PacketOut)
            .GetVal(V));
      End;
      If (Key = 'in') Then
      Begin
        If TryStrToInt(Val, V) Then
          LBIn.Items.Add('0x' + IntToHex(V, 3) + ' - ' + NLU(LU_PacketIn)
            .GetVal(V));
      End;
  ...
  Except
    on E: Exception do
      MessageDlg('Failed to load ' + FN + #10#13 + E.Message, mtError,
        [mbAbort], 0);
  End;
  FreeAndNil(Line);
  FreeAndNil(SL);
...
Begin
  ...
  Try
    ...
    If (RBOutOff.Checked) Then
      SL.Add('outtype;off');
    If (RBOutHide.Checked) Then
      SL.Add('outtype;hide');
    If (RBOutShow.Checked) Then
      SL.Add('outtype;show');
    If (RBOutNone.Checked) Then
      SL.Add('outtype;none');
    ...
    If (RBInOff.Checked) Then
      SL.Add('intype;off');
    If (RBInHide.Checked) Then
      SL.Add('intype;hide');
    If (RBInShow.Checked) Then
      SL.Add('intype;show');
    If (RBInNone.Checked) Then
      SL.Add('intype;none');
  ...
  Except
    on E: Exception do
      MessageDlg('Failed to save ' + FN + #10#13 + E.Message, mtError,
        [mbAbort], 0);
  End;
  FreeAndNil(SL);
End;
